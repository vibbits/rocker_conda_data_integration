...
MOFAobject
```
## option 3: bring your own data
If one wants to use their own data 
I suggest using the magrittr package to create data list
...
## In order to save your trained model for later use: 
save(MOFAobject, file = "/home/rstudio/kitematic/model.hdf5")
```
########################################
########################################
```{r}
...
plotWeightsHeatmap(
  MOFAobject, 
  view = "Mutations", # change here to "grugs"" for example, and see what happen
  factors = 1:5,
  show_colnames = TRUE
)
```
